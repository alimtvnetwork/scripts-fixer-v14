...
has_jq || { log_err "[26] jq required to read config"; exit 1; }

APT_PKG=$(jq -r '.install.apt' "$CONFIG")
VERIFY_CMD='neo4j --version'
...
INSTALLED_MARK="$ROOT/.installed/26.ok"

verify_installed() { bash -c "$VERIFY_CMD" >/dev/null 2>&1; }

setup_repo() {
  has_curl || { log_err "[26] curl required"; return 1; }
  command -v gpg >/dev/null 2>&1 || { sudo apt-get install -y gnupg || return 1; }
  log_info "[26] Fetching GPG key from $KEY_URL"
  local key_tmp
  key_tmp=$(mktemp /tmp/26-install-neo4j.gpg.XXXXXX) || { log_file_error "/tmp" "mktemp failed"; return 1; }
  ...
  rm -f "$key_tmp"
  log_info "[26] Writing apt repo: $REPO_LIST"
...
verb_install() {
  ...
  setup_repo || return 1
  log_info "[26] Installing via apt: $APT_PKG"
  if sudo apt-get install -y $APT_PKG; then
    log_ok "[26] Installed"; mkdir -p "$ROOT/.installed"; touch "$INSTALLED_MARK"; return 0
  fi
  log_err "[26] apt install failed"; return 1
}
verb_check()     { if verify_installed; then log_ok "[26] Verify OK"; return 0; fi; log_warn "[26] Verify FAILED"; return 1; }
verb_repair()    { rm -f "$INSTALLED_MARK"; verb_install; }
verb_uninstall() {
  sudo apt-get remove -y $APT_PKG || true
  sudo rm -f "$REPO_LIST" "$KEYRING"
  rm -f "$INSTALLED_MARK"
  log_ok "[26] Removed (apt + repo file + keyring)"
}
case "${1:-install}" in install) verb_install;; check) verb_check;; repair) verb_repair;; uninstall) verb_uninstall;; *) log_err "[26] Unknown verb: $1"; exit 2;; esac
