...
Write-InstallPaths `
    -Tool   "LiteDB" `
    -Source "https://www.nuget.org/packages/LiteDB (NuGet)" `
    -Temp   ($env:TEMP + "\scripts-fixer\litedb") `
    -Target ($env:LOCALAPPDATA + "\litedb")
...
if ($isUninstall) {
    Uninstall-Litedb -DbConfig $config.database -LogMessages $logMessages
    return
}
