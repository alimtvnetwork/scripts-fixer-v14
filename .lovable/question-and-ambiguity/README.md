...
| 10 | [10-install-bootstrap-version-flag.md](./10-install-bootstrap-version-flag.md) | Verify + implement working `--version` across `install.ps1` / `install.sh` with consistent reporting | **Option B** — pre-existing `--version` only reported bootstrap vN; extended both sides to also fetch + report payload semver from `scripts/version.json`, added matching `--help` / `-Help`, harmonised `[VERSION]` / `[FOUND]` / `[RESOLVED]` labels & order. Verified live against scripts-fixer-v13/v10 under bash and pwsh 7.5.4. | open |
